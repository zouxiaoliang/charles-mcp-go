...
**让 AI 客户端使用 Charles 抓包、定位接口错误，并修改参数重放请求。**

[下载正式版](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest) · [客户端配置](docs/clients.md) · [场景教程](docs/tutorials/README.md) · [可复现演示](examples/README.md)

![真实 MCP 调用：导入、定位 401、重放得到 200](docs/assets/demo.gif)

演示来自本地示例 API 上实际执行的 MCP 调用，使用示例凭据；动画按真实输出排版，不是 AI 客户端或 Charles 界面的录屏。
...
使用预编译文件无需安装 Go。通过 `go install` 或源码构建时需要 Go 1.26 或更新版本；项目固定推荐工具链 1.26.8。

### 下载可执行文件（无需 Go）

| 系统 | amd64 / Intel | arm64 / Apple Silicon |
|---|---|---|
| macOS | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-darwin-amd64) | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-darwin-arm64) |
| Linux | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-linux-amd64) | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-linux-arm64) |
| Windows | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-windows-amd64.exe) | [下载](https://github.com/zouxiaoliang/charles-mcp-go/releases/latest/download/charles-mcp-windows-arm64.exe) |

macOS/Linux 下载后执行 `chmod +x <下载的文件>`，再改名为 `charles-mcp`。Windows 可改名为 `charles-mcp.exe`。运行 `--version` 验证；`--check` 用于连接已有 Charles，需要先完成其配置。
...
首次体验可以让客户端导入仓库中的 `examples/session.json`，按 `errors_only` 过滤并解释两个 401 响应，不需要启动 Charles。[三个场景教程](docs/tutorials/README.md) 包含具体提示词、工具参数和预期结果。

```text
导入 /你的绝对路径/charles-mcp-go/examples/session.json。
找出失败的请求，展示响应中的错误信息，并解释下一步需要检查什么。
```

如果项目对你有帮助，欢迎 Star；遇到安装或抓包问题，请通过 [Issue](https://github.com/zouxiaoliang/charles-mcp-go/issues/new/choose) 提供复现步骤。

## 客户端配置

Claude Desktop 和 Cursor 的配置路径及可复制示例见 [客户端接入指南](docs/clients.md)。
